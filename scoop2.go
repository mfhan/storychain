...
func (t *SimpleChaincode) firstedit(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
    var story string//  , value string
    var err error
    fmt.Println("running edit()")

    story = args[0]
    valAsbytes, err := stub.GetState(story)
    if err != nil {
//        jsonResp = "Story does not exist"
//        return nil, errors.New(jsonResp)
    ...
    // State logic
    if (val != "written") {
        return nil, errors.New("Only written stories can be edited")
    }

//    err = stub.PutState(key, []byte("firstedited"))  //write the variable into the chaincode state
    if err != nil {
        return nil, err
    }
    return nil, nil
...
//        return nil, errors.New(jsonResp)
    ...
    // State logic
    if (val != "edited") {
        return nil, errors.New("Only edited stories can be approved")
    }

//    err = stub.PutState(key, []byte("approve"))  //write the variable into the chaincode state
    if err != nil {
        return nil, err
    }
    return nil, nil
